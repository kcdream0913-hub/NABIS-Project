...
| D-052 | 2026-07-28 | **DM attachment type is decided SERVER-SIDE by MAGIC BYTES, never the client Content-Type or the filename extension (BL-MSG-05).** `lib/attachmentSniff.ts` sniffs an allowlist (jpeg/png/webp/gif · mp4/webm · pdf/docx/xlsx · csv/txt) and REJECTS executables/installers/archives outright (MZ, ELF, Mach-O, shebang, RAR, 7z, gzip, bzip2, xz, and any non-OOXML ZIP). **ZIP discrimination:** docx/xlsx are ZIP containers (`PK\x03\x04` — identical magic to a generic `.zip`), so a bare "reject ZIP" would reject every Office doc; instead, on `PK` we scan the head window for the OOXML `[Content_Types].xml` local-file entry plus a `word/` (docx) or `xl/` (xlsx) part — anything else `PK` = generic archive = REJECT. **csv/txt have no signature** → a strict UTF-8-text heuristic (no NUL, no C0/C1 controls except tab/CR/LF) → `text/plain`; a binary masquerading as `.txt` fails the text check and is rejected. **Mismatch policy = the extension is IGNORED for the security decision.** A file is accepted iff its *sniffed* type is allowlisted; it is then stored under a generated UUID key with the SNIFFED type recorded — never the client's claim. So `invoice‮gpj.exe` (magic MZ) is rejected regardless of a `.pdf`-looking name; there is no "accept-and-rename to match the lie." **Enforcement is unskippable:** the sniff runs in the signed-URL READ route BEFORE minting a URL, so even a message inserted by a malicious client referencing an un-scanned `.exe` can never be DELIVERED (403, no URL). Displayed filenames are sanitized (strip U+202A–202E / U+2066–2069 / other bidi + control chars, cap length, rendered as text never HTML) at BOTH store and render, because a malicious sender can write the jsonb directly. | A fraud-protection marketplace's DM attachments are a direct malware/fraud vector; Content-Type and extension are attacker-controlled, so only the bytes can be trusted, and the check must sit on the unskippable delivery path, not just the composer. |
| D-053 | 2026-07-28 | **`message-attachments` bucket expanded for BL-MSG-05 (migration, committed NOT applied).** Finding that corrected the spec: the DM bucket was **10MB** (not the 50MB cited — that is the *feed* video bucket) and its `allowed_mime_types` was jpeg/png/webp/pdf/docx/xlsx only, so it **physically rejected** video/gif/csv/txt uploads — "Photos & videos" + csv/txt were impossible without a bucket change. Migration raises `file_size_limit` to 50MB (to allow video ≤90s; images still capped smaller client-side) and expands `allowed_mime_types` to the Phase-1 set. The Content-Type allowlist is only a coarse first filter (client-controlled); the real type boundary is the D-052 magic-byte sniff on read. Storage RLS is UNCHANGED (already participant-scoped: `message_attach_select/insert/delete` on `private.is_thread_participant`). The same migration bundles the four pending BL-NOTIF-01 `revoke execute` statements (per `notif-trigger-fn-revoke-pending.md`) since it is the next DB migration. | Spec assumed the feed bucket's 50MB/mime set; the DM bucket differs and blocks the requested types — a one-statement config change is the minimal way to meet the scope, and it is the natural place to land the deferred notif revokes. |
