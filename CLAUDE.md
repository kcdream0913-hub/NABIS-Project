...
- **PRE-PILOT HARD BLOCKER — E2E must run against a SEPARATE Supabase project, NOT
  ...
  **IN PROGRESS (BL-E2E-SPLIT-01 / D-085):** the code/doc artifacts are on branch
  `bl-e2e-split-01` (seed script, CI repoint + D-062 relax, baseline plan) — NOT merged.
  Still GATED on KC: the `supabase db dump` schema baseline (needs the prod DB password),
  creating the E2E project, setting the new GitHub secrets, and a CI run. **This blocker
  CLOSES only when a zero-delta prod snapshot (before/after a full CI E2E run) is green** —
  not before.
...
| D-085 | 2026-08-04 | **BL-E2E-SPLIT-01 — move the E2E suite off PRODUCTION onto a dedicated Supabase project (`sangamline-e2e`). The pre-pilot hard blocker ("no pilot user is onboarded until this is done"). IN PROGRESS — artifacts on branch `bl-e2e-split-01` (NOT merged); several steps are GATED on KC.** **The hidden first step = schema parity, NOT project creation.** Measured live 2026-08-04 (hub re-derived): prod has **53** recorded migrations; the repo spreads them across THREE conventions — `supabase/migrations/` (dead since **2026-07-28**), `supabase/migrations_archive/`, and `docs/BL-*.sql` — with everything from `bl_event_01` onward (**7** prod migrations) living in `docs/` or nowhere. (This task adds one repo-provenance file, `supabase/migrations/20260804043523_avatars_scoped_select_own.sql`, for the D-084 policy the hub applied to prod ahead of merge — see D-084.) CLAUDE.md has warned since 2026-07-22 that applying the repo to an empty DB does NOT reproduce prod — still true, gap widened. Standing up E2E from the repo's migrations would produce a DB that SILENTLY differs, and tests that pass against the wrong schema are worse than none. So **step 1 is a faithful `supabase db dump` from prod** — schema-only is the DEFAULT (do NOT pass `--data-only`; there is NO `--schema-only` flag; `--linked` if the project is linked) — committed as a new baseline `supabase/migrations/00000000000001_baseline_2026_08_04.sql` that SUPERSEDES the archived + `docs/BL-*` files (retire them to `migrations_archive/`); **one convention going forward: `supabase/migrations/`**. Full executable procedure — dump command, post-dump grep proofs (`CREATE SCHEMA private` present, `insert into storage.buckets` ABSENT), the verified storage-half file, the pre-restore **privilege trap** (`alter default privileges in schema public revoke all on tables from anon, authenticated` — skip it and the E2E DB is MORE permissive than prod, so RLS tests pass that should fail), extension asserts, and the count table — lives in `docs/BL-E2E-SPLIT-01-baseline-runbook.md` (every value re-verified against prod 2026-08-04). Verification target (what the baseline must reproduce — read live): **35** public tables, **105** RLS policies (95 public + 10 storage), **7** `private` helpers, **23** public functions, **15** triggers, **3** storage buckets, generated columns on `profiles`. **Storage needs explicit handling** — `supabase db dump` EXCLUDES the `storage` (and `auth`) schema BY DESIGN (managed schema), so the dump carries NONE of the 3 buckets or 10 storage policies — the single most likely SILENT failure (attachments then fail looking like a code bug). The storage half is a SECOND file `supabase/migrations/00000000000002_baseline_storage_2026_08_04.sql` (verified byte-exact vs prod: `post-media` private 50MB, `message-attachments` private 50MB 11 mime, `avatars` **public** 2MB jpeg/png/webp, + all 10 policies) that lands ATOMICALLY WITH file 1 + the retirement — NOT committed before, because it references `private.*` fns file 1 creates and would double-create the D-084 `avatars_select_own` still live in `20260804043523_…sql` (which the retirement archives). Verified content + reasoning: the runbook. Verify the baseline by applying to a DISPOSABLE Supabase branch and diffing object counts against prod (assert the 95 public and 10 storage policies SEPARATELY — the storage 10 are the least likely to survive a `db dump`) before proceeding — but counts are a WEAK gate (all pass even if `posts_select` restored as `using(false)`); the AUTHORITATIVE equivalence check is a 9-hash parsed-definition **fingerprint**, `supabase/migrations/BASELINE_FINGERPRINT.md` (re-verified against prod 2026-08-04 — all 9 match), where nine md5 matches = the schema is the same schema. **Seeding trap (`scripts/seed-e2e.mjs`, committed, repeatable):** accounts A/B/C **MUST be verified** — BL-TRUST-01 (D-082) gates posting/commenting/quote-reposting on `verification_status='verified'`; set it via the BASE column **`us_verification='verified'`**, NEVER `verification_status`/`verified_at`/`bridge` (**428C9** — generated columns). Also seeds the A<->B thread (→ `THREAD_AB`, the ONLY code change, deferred until the script prints the new id) + a B<->C foreign attachment object (→ `E2E_FOREIGN_ATTACHMENT_PATH`); the script hard-refuses the prod project ref. **CI (committed, NOT merged):** the `e2e` job repoints `NEXT_PUBLIC_SUPABASE_URL`/`ANON_KEY` to new `E2E_SUPABASE_URL`/`E2E_SUPABASE_ANON_KEY` secrets; the `unit` job KEEPS prod's NEXT_PUBLIC_* (build parity). **D-062 decision: KEEP a constant `group: e2e`** (was `e2e-prod`) — it no longer blocks prod work, but is kept because the suite still shares one account A + THREAD_AB within the E2E project, so parallel runs would corrupt each other via global-teardown; per-ref PARALLELISM would need the run-scoped object/message prefixes D-062 describes — deferred, stated. **GATED ON KC (the coding agent CANNOT do these):** (1) run `supabase db dump --db-url <prod conn string>` — needs the prod DB password, a SECRET the agent must not hold; (2) create the billable E2E project + apply the baseline; (3) set the new GitHub secrets; (4) trigger the CI run. **PROOF (the only one that matters):** snapshot prod counts (`posts/post_comments/post_reactions/post_reposts/post_bookmarks/messages/storage.objects`) BEFORE and AFTER a full CI E2E run — IDENTICAL = done; anything else means a path still points at prod. Also confirm the suite PASSES (not skips — the D-063 failure mode) against the new project. The PRE-PILOT HARD BLOCKER closes only when a zero-delta run is green. gates: tsc 0 · vitest 406/406 · build 0 both locales (no app-code change; the seed is a node `.mjs`). findings→permanent checks: the zero-delta prod snapshot (standing proof each run), the seed script's prod-ref refusal guard, and the dedicated project itself (structural — it removes the shared-mutable-prod-state class entirely). NOTE (KC, separate pre-launch step): the 30 test accounts already in prod STAY — this stops NEW pollution; `scripts/delete-test-accounts.mjs` cleans the old, run once right before onboarding real members. | The suite writing real content into the live 6-real-member project degrades with every task shipped; teardown/Option-C/D-062 only BOUND the exposure. The real fix is a separate database — but the load-bearing risk is schema parity: a repo that can't rebuild prod yields an E2E DB that silently differs, so a faithful prod dump is the true step 1, and it plus the billable project + secrets are inherently KC's to run. |
