...
- **The "verified to post" gate — NOW SERVER-SIDE for CONTENT (posts + post_comments) via
  ...
  `post_comments_insert_own`, and gates the comment composer client-side. Reactions / reposts
  / bookmarks stay UNGATED on purpose (D-082): they were never client-gated either, 76% of
  current reactions are by unverified users, and blocking the cheapest engagement action for
  near-zero abuse value is a live cost pre-critical-mass — gating them is a separate product
  decision, not a gap. Migration WRITTEN, not applied — hub verifies in begin/rollback then
  applies. **Still open (D-082 question for KC):** a post published *as a business* gates on
  the HUMAN `author_id`, not the business's own `verification_status`.
